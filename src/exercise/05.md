...
? What would be a use case for `useImperativeHandle`?

-> This is an imperative approach, that should only be used for cases that can't
be expressed as props. These would be scrolling to a node, focusing a node,
triggering an animation, selecting text, etc. Declarative way is otherwise
better.
