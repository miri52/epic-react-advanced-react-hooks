...
? When should you use `useLayoutEffect` instead of `useEffect`?
    ...
    -> When you are making observable changes to the DOM. In such case with useEffect the user could see a flicker when your DOM mutations take effect. However, this has been smoothed out in React 18. Using useLayoutEffect also can hurt the performance as it renders in 2 passes and blocks te browser. Prefer useEffect when possible.
