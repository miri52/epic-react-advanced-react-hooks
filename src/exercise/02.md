...
? What is memoization? Why would we want to memoize something?

-> To improve performance by avoiding expensive computations (re-rendering
expensive components or calling expensive functions) and for value stability

? What is the difference between `useCallback` and `useMemo`?

-> `useMemo` lets you apply memoization to any value, not just functions (which
is the case of `useCallback`)
