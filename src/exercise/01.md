...
? When would we use a reducer?

-> When we have components with many state updates across many event handlers.
This can be hard to read and overwhelming. For those cases we can consolidate
all the state update logic outside the component in a single function, called a
reducer.

? What arguments does the reducer function take? And what does it return?

-> 2 arguments - the first argument is the current state and the second argument
is the action object. (Action is whatever is passed to the updater/dispatch
function). The reducer returns the next state for React to set

? What arguments does the useReducer take? And what does it return?

-> 2 arguments - a reducer function and an initial state. It returns a stateful
value and a dispatch function, to which we pass the action object.
